...
|--------|-------|-----------|
| AV (Attack Vector) | N (Network) | Attack is executed via P2P ShrEx protocol over the network |
| AC (Attack Complexity) | H (High) | Requires malicious peer in victim's peer table and victim selecting that peer; one-shot then blacklisted |
| PR (Privileges Required) | N (None) | Any peer with P2P access can attempt the attack |
...
| A (Availability) | L (Low) | Transient memory spike recoverable by GC; OOM improbable on recommended hardware |
