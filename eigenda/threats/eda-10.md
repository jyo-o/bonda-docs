# EDA-10: Configuration Flag Can Disable All Anchor Signature Verification
...
An attacker with access to the Disperser server's environment variables or configuration can set `DisableAnchorSignatureVerification` to `true`, completely bypassing anchor signature verification. This would enable cross-chain replay attacks (related to EDA-03) by removing the chain-binding protection.
...
Affects the Verifiability and Decentralization baselines. A privileged configuration flag can disable anchor signature verification, so this is recorded as a Governance Observation and carries no score.
...
2. Set `TolerateMissingAnchorSignature` to `false` in production to enforce anchor signatures (related to EDA-03).
